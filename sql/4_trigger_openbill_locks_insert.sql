...
BEGIN
  ...
  IF NEW.amount_cents < 0 THEN
    SELECT amount_cents FROM OPENBILL_HOLDS WHERE key = NEW.lock_key INTO v_lock_amount;
    SELECT SUM(amount_cents) FROM OPENBILL_HOLDS WHERE lock_key = NEW.lock_key INTO v_unlock_amount;
