...
DECLARE
 ...
 v_account_to OPENBILL_ACCOUNTS%rowtype;
BEGIN
  SELECT * FROM OPENBILL_ACCOUNTS WHERE id = NEW.from_account_id FOR UPDATE INTO v_account_from;
  SELECT * FROM OPENBILL_ACCOUNTS where id = NEW.to_account_id and amount_currency = NEW.amount_currency FOR UPDATE INTO v_account_to;
